...
.elseif $0 == SUPER2_STRET
	...
	tst	r12, r12		// set ne for forwarding (r12!=0)
	MESSENGER_END_FAST
	bx	r12			// call imp  // 利用 IMP 调用方法的函数实现
.else
.abort oops
.endif
...
.macro CacheLookup    // 标记一下，objc_msgSend 查找方法缓存的实现在这里
...
// objc-cache.h 中 IMP cache_getImp(Class cls, SEL sel) 方法的实现
...
/*
 ...
 1. 判断receiver是否为nil，也就是objc_msgSend的第一个参数self，
      也就是要调用的那个方法所属对象
 2. 从缓存里寻找，找到了则分发，否则
 3. 利用 objc-runtime-new.mm 中_class_lookupMethodAndLoadCache3 方法去寻找selector
 ...
 */
	...
	CacheLookup NORMAL  // 查找缓存，也查找就是 objc_class 中的 cache 是否 sel 对应的 IMP
 ...
 *
    这句不知道怎么翻译，不好理解。感觉这个函数是在缓存未命中时，做一些事，比如查找方法列表等
...
// 调用 _class_lookupMethodAndLoadCache3 函数查找方法，如果找到就将方法加载到缓存中，该函数的实现在 objc-runtime-new.mm 中
