...
* If the data doesn't exist yet and create is YES, allocate and return it.
 ...
 取得这个线程的 线程数据
 如果 data 不存在，并且 create == NO，就返回 NULL
 如果 data 不存在，但是 create == YES，就为 data 开辟一块内存，
      用 TLS 存起来， 然后将它返回
**********************************************************************/
_objc_pthread_data *_objc_fetch_pthread_data(bool create)
{
    ...
    // 用 _objc_pthread_key 为 Key 从 TLS 从取 data
    data = (_objc_pthread_data *)tls_get(_objc_pthread_key);
    // 如果 data 不存在，但是 create == YES
    if (!data  &&  create) {
        // 就为 data 开辟一块内存
        data = (_objc_pthread_data *)
            calloc(1, sizeof(_objc_pthread_data));
        // 用 _objc_pthread_key 为 Key 在 TLS 把 data 存起来
