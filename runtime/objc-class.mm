...
Class object_setClass(id obj, Class cls)
{
    ...
    // 为了防止弱引用 the weak reference machinery 和 +initialize machinery 之间出现死锁，
    // 就必须确保被弱引用的对象的 isa 都被 +initialized 过
    
    // 如果 cls 类既没有 future ，也没有 Initialized
 ...
 **********************************************************************/
// 在 obj 对象中寻找指定的 SEL 对应的 IMP，这个 IMP 的返回值是一个结构体
#if SUPPORT_STRET
...
* Does not check if the method already exists.
 ...
 // resolve cls 类中的指定的 SEL，inst 是 cls 类或其子类的实例
 // 会根据 cls 是否是元类，调用不同的方法 [cls resolveInstanceMethod:sel] 或者 [nonMetaClass resolveClassMethod:sel]
 // 什么都不返回，任何结果都将可能是过期的。原因是 lookUpImpOrForward 里在进行 resolve 前，就已经释放了读锁，所以中间很有可能有其他线程已经做了其他的操作，导致结果过期无效
 // 也不检查方法是否已经存在
 // 该函数只被 lookUpImpOrForward() 一个函数调用
**********************************************************************/
void _class_resolveMethod(Class cls, SEL sel, id inst)
{
    if (! cls->isMetaClass()) { // 如果 cls 不是元类，即它是一个实例类，就尝试调用 +resolveInstanceMethod
        // try [cls resolveInstanceMethod:sel]
        _class_resolveInstanceMethod(cls, sel, inst);// 在里面会调用 +resolveInstanceMethod
                                                     // +resolveInstanceMethod 里，我们可以为 cls 类动态地添加实例方法
    } 
    else { // 如果是元类
        // try [nonMetaClass resolveClassMethod:sel]
        // and [cls resolveInstanceMethod:sel]
        
        // 就先尝试调用 +resolveClassMethod，在 +resolveClassMethod 我们可以动态地为 cls 类动态地添加类方法
        _class_resolveClassMethod(cls, sel, inst);
        if (!lookUpImpOrNil(cls, sel, inst, // 添加完类方法后，再去 cls 类中搜索 sel 对应的 IMP，如果还是没有找到
                            NO/*initialize*/, YES/*cache*/, NO/*resolver*/))
                                                        // 这里必须设置不 resolver，否则就死循环了
        {
            // 就只能退而求其次，尝试添加实例方法
...
// 从 cls 类中查找指定的 SEL 对应的 IMP
// 如果没找到，就返回消息转发函数 _objc_msgForward()
IMP class_getMethodImplementation(Class cls, SEL sel)
{
    ...
    // 利用 lookUpImpOrNil 函数在 cls 中寻找 sel 对应的 IMP
    // initialize YES : 如果类没有初始化，就将类初始化了
    // cache YES : 先从缓存中查找
    // resolver YES : 如果在 cls 的缓存和方法列表、祖宗类的缓存和方法列表中都没找到，就尝试 resolve，详情见 lookUpImpOrNil()
    // lookUpImpOrNil 如果没有找到的话，会返回 nil
...
#if SUPPORT_STRET
// 在 cls 中寻找指定的 SEL 对应的 IMP，该 IMP 的返回值是结构体，
// 这是与 class_getMethodImplementation 不同的
IMP class_getMethodImplementation_stret(Class cls, SEL sel)
{
    // 内部调用的也是 class_getMethodImplementation 函数
    IMP imp = class_getMethodImplementation(cls, sel);

    // 不同的是，如果 class_getMethodImplementation 返回 IMP 是 _objc_msgForward，
    // 就返回 _objc_msgForward_stret
